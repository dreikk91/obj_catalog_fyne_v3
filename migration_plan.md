...
| Робоча область об'єкта | ~60% | Картка, зони, контакти, lazy-журнал об'єкта, експорт PDF/XLSX/clipboard; зони згруповані в `QTreeView`; потрібне подальше полірування UX |
| Загальні тривоги | ~60% | Операційна стрічка з групуванням по об'єкту/джерелу, станом, лічильниками, швидкими фільтрами, multi-select діями й хронологією; потрібне подальше полірування flow |
| Загальний журнал подій | ~50% | Таблиця, кольори, номер об'єкта, фільтри/пауза, контекстне меню, лічильник вкладки; потрібне подальше полірування |
...
- Реалізовано список об'єктів:
  ...
  - фільтр стану
  - фільтр джерела
  - пошук і вибрані фільтри списку зберігаються в Qt preferences.
  ...
  - контекстне меню рядка з відкриттям картки та копіюванням номера, назви, адреси або повного рядка.
  - стабілізовано програмний вибір об'єкта: прибрано повторний callback вибору та небезпечний `QStandardItemModel.Index` у перевірці поточного рядка.
- Реалізовано робочу область:
  - вкладка `Картка`
  - вкладка `Зони`
  - вкладка `Контакти`
  - вкладка `Журнал`
  - вкладка `Експорт` з PDF, XLSX, clipboard TSV і додаванням до звіту видалених об'єктів.
- Картка об'єкта перероблена на прокручувану форму:
  - `QScrollArea`
  - read-only `QLineEdit`
  - широкі рядки для назви, адреси, розташування, SIM
  - довгі поля винесені на всю ширину форми, додані tooltip-и для повного тексту
...
- Журнал об'єкта:
  ...
  - події вантажаться lazy тільки при відкритті вкладки `Журнал`;
  - повторне відкриття вкладки не запускає дубльоване завантаження для того самого об'єкта;
  - експорт PDF/XLSX і звіт видалених об'єктів дозавантажують події, якщо вкладка `Журнал` ще не відкривалась.
- Загальний журнал подій:
  - додано номер об'єкта
  - додано кольори подій
  - автопідбір/збереження ширин колонок.
  - додано фільтри періоду, джерела, severity (`Всі`, `Критичні`, `Попередження`, `Інформаційні`) та режим `По вибраному`.
  - вибрані фільтри журналу зберігаються в Qt preferences.
  - додано паузу/продовження оновлення.
  - додано гарячу клавішу `Ctrl+P` для паузи/продовження журналу.
  - додано гарячу клавішу `Ctrl+Shift+O` для режиму `По вибраному`.
  - додано контекстне меню з копіюванням опису та переходом до об'єкта.
  - нижня вкладка `Журнал подій` показує кількість подій після поточних фільтрів.
- Загальна таблиця тривог:
  - додано номер об'єкта
  - додано кольори критичних тривог
  - автопідбір/збереження ширин колонок.
  - нижня вкладка `Тривоги` показує кількість тривог після поточних фільтрів.
- Стрічка тривог перероблена в операційний блок:
  - одна строка тепер є групою активних тривог по `джерело + об'єкт`, а не окремою подією;
  - у групі показано кількість тривог, кількість критичних, зону та ключовий тип/деталі;
  - верхня панель зроблена компактною, щоб основну висоту займала таблиця груп;
  - помітний статус активних/критичних тривог;
  - кольорові лічильники `Критичні`, `Звичайні`, `Груп`, `Вибрано`;
  - швидкі фільтри `Критичні`, `Всі`, `Звичайні`;
  - кнопки `Відпрацювати` і `Взяти в роботу` активні тільки при вибраних групах;
  - дії по рядку застосовуються до всіх тривог вибраної групи;
  - multi-select кількість тривог відображається прямо на кнопках дій;
  - хронологія більше не відкривається при простому кліку по рядку; вона показується кнопкою/контекстним меню;
  - таблиця отримала читабельніший header/selection style.
- Qt-панель тривог отримала базовий MVP flow:
  ...
  - діалог групового відпрацювання показує кількість і список вибраних тривог, а не лише першу тривогу.
  - контекстне меню тривог застосовує дії до всього виділення, якщо правий клік зроблено по виділеному рядку.
...
- Додано простий SIM dialog:
  ...
  - кнопки надсилання SMS на SIM1/SIM2 через Omnicell API.
- Додано Omnicell SMS інтеграцію:
  - окремий `pkg/omnicell` клієнт для IP2SMS XML API;
  - налаштування endpoint/login/password/source у Qt settings;
  - нормалізація українських номерів SIM до `380...`.
  - профілі SMS для SIM: `МЦА-GSM`, `МЦА-GSM4`, `Вільний текст`;
  - для `МЦА-GSM` генеруються дві стандартні SMS з шаблонів старого Python UI;
  - для `МЦА-GSM4` генеруються SMS №1/№2 і коротка SMS №3 з об'єктовим номером, прихованим номером, APN, IP, портами та інтервалами тесту.
- Додано Qt Designer прототипи в `pkg/qtui/designer`:
  - `main_window.ui`
  - `object_list_panel.ui`
  - `alarm_panel.ui`
  - `event_log_panel.ui`
  - `work_area_panel.ui`.
...
- Нормалізовано Qt DB settings:
  - порожні `host`/`port`/`path` більше не перекривають fallback-значення
  - Firebird DSN будується з trimmed host/port/path.
- Refresh scheduler/eventbus більше не викликає Qt UI напряму з фонового goroutine; refresh доставляється через main-thread queue.
- Перевірки, які проходили під час роботи:
  ...
  - `go test -tags qt ./pkg/qtui ./pkg/qtapp ./pkg/config`
...
- Полірувати контекстні меню для об'єктів і подій.
- Полірувати фільтри загального журналу подій:
  - тонше розділення типів подій всередині severity.
- Додати додаткові дії у таблиці тривог:
  - групове відпрацювання
  - деталі/історія тривоги.
- Полірувати вкладку експорту об'єкта:
  - показ прогресу/статусу довгих операцій
  - вибір шляху для звіту видалених об'єктів
  - краща обробка скасування/помилок.
...
4. Полірувати вкладку експорту об'єкта.
