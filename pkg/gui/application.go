...
func NewApplication() *Application {
	...
	log.Info().Msg("Ініціалізація backend runtime...")
	backendRuntime := backend.NewRuntime(dbCfg)
	dataProvider := backendRuntime.Provider()
	log.Debug().Msg("Backend runtime готовий")
...
func (a *Application) Run() {
	...
	if a.backend != nil {
		defer func() {
			log.Debug().Msg("Закриття backend runtime...")
			a.backend.Close()
			log.Debug().Msg("✓ Backend runtime закрито")
		}()
	}
...
func (a *Application) Reconnect(cfg config.DBConfig) {
	...
	if a.backend == nil {
		err := fmt.Errorf("backend runtime is not initialized")
	...
	log.Debug().Msg("Запит на reconnect backend runtime...")
	if err := a.backend.Reconnect(cfg); err != nil {
		log.Error().Err(err).Msg("❌ Помилка перевірки з'єднання з новою БД")
		if a.statusLabel != nil {
			a.statusLabel.SetText("БД : помилка підключення")
		}
	...
	a.dataProvider = a.backend.Provider()
